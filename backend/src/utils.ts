...
export function validateCatalogs(catalogs: unknown): SelectedCatalog[] {
	if (!Array.isArray(catalogs)) return defaultSelectedCatalogs;

	const valid = catalogs
		.map((entry) => (typeof entry === 'string' ? { catalog: entry } : entry))
		.filter((entry) => entry && allCatalogs.includes(entry.catalog))
		.map(({ catalog, sort }) => ({
			catalog,
			sort: resolveSort(catalog, sort),
		}));

	const byCatalog = new Map(valid.map((entry) => [entry.catalog, entry]));
